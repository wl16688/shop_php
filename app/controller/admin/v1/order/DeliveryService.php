...
class DeliveryService extends AuthController
{
    ...
    {
        return $this->success($this->services->getServiceList(['type' => 0, 'is_del' => 0]));
    }
    ...
    public function save()
    {
        ...
        if ($this->services->count(['uid' => $data['uid'], 'type' => 0, 'is_del' => 0])) {
            return $this->fail('配送员已存在!');
        }
        if ($this->services->count(['phone' => $data['phone'], 'type' => 0, 'is_del' => 0])) {
    ...
    public function update($id)
    {
        ...
        if ($delivery['phone'] != $data['phone'] && $this->services->count(['phone' => $data['phone'], 'type' => 0, 'is_del' => 0])) {
            return $this->fail('同一个手机号的配送员只能添加一个!');
        }
