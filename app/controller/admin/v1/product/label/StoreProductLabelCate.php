...
class StoreProductLabelCate extends AuthController
{
    ...
    public function save()
    {
        ...
        if ($this->services->getOne(['name' => $data['name'], 'group' => 2, 'type' => 0])) {
    ...
    public function update($id)
    {
        ...
        $cate = $this->services->getOne(['name' => $data['name'], 'group' => 2, 'type' => 0]);
        if ($cate && $cate['id'] != $id) {
            return $this->fail('标签组已存在');
        }
    ...
    public function delete(StoreProductLabelServices $labelServices, $id)
    {
        ...
        if ($id == 1) {//系统默认分组 不允许删除
			return $this->fail('系统默认标签组不能删除');
		}
