...
class StoreProductUnit extends AuthController
{
    ...
    /**
     * @var StoreProductUnitServices
     */
    #[Inject]
    protected StoreProductUnitServices $services;
    ...
    {
        return $this->success($this->services->getAllUnitList());
    }
    ...
    public function save(Request $request)
    {
        ...
        if ($this->services->getCount(['name' => $data['name'], 'is_del' => 0, 'type' => 0])) {
    ...
    public function update(Request $request, $id)
    {
        ...
        $unit = $this->services->getOne(['name' => $data['name'], 'is_del' => 0, 'type' => 0]);
        if ($unit && $unit['id'] != $id) {
	 ...
	 * @throws \think\db\exception\ModelNotFoundException
	 */
    public function delete(StoreProductServices $productServices, $id)
    {
        if (!$id || !($info = $this->services->get($id))) {
            return $this->fail('删除的数据不存在');
        }
		if ($productServices->getCount(['unit_name' => $info['name'], 'is_del' => 0])) {
			return $this->fail('该单位正在使用，不能删除');
		}
