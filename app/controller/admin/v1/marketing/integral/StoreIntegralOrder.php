...
use app\services\order\StoreOrderServices;use app\services\serve\ServeServices;
...
class StoreIntegralOrder extends AuthController
{
    ...
    public function get_express($id, ExpressServices $services)
    {
        ...
        $data['result'] = $services->query($cacheName, $orderInfo['delivery_id']);
        return $this->success($data);
    }
    ...
    {
        if (!$id) return $this->fail('缺少参数');
        $res = $this->services->orderPrint((int)$id);
        if ($res) {
