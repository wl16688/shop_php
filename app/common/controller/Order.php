...
trait Order
{
    ...
    public function write_update(StoreOrderWriteOffServices $services, $order_id)
    {
        ...
        if (!$orderInfo) {
            return app('json')->fail('订单不存在或已被删除');
        }

        if ($orderInfo->shipping_type != 2 && $orderInfo->delivery_type != 'send') {
            return app('json')->fail('该订单不支持核销');
        } else {
            if (!$orderInfo->verify_code) {
                return app('json')->fail('核销码缺失');
            }
            ...
            try {
                ...
                if ($orderInfo) {
                    ...
                    return app('json')->fail('核销失败，请稍后重试');
                }
            } catch (\Exception $e) {
                return app('json')->fail('核销过程中发生错误：' . $e->getMessage());
            }
        }
    }
    ...
    public function getUpdateInfo(StoreOrderCartInfoServices $cartInfoServices, $id)
    {
        ...
        foreach ($cartInfoList as $cart) {
            $decodedCart = is_string($cart) ? json_decode($cart, true) : $cart;
            if (json_last_error() === JSON_ERROR_NONE) {
                $cartInfo[] = $decodedCart;
            } else {
                return app('json')->fail('订单商品信息解析失败!');
            }
        }
    ...
    public function del_orders(Request $request)
    {
        ...
        if ($all == 0) {
            if (!$ids) return app('json')->fail('请选择需要删除的订单');
            if ($this->services->getOrderIdsCount($ids)) return app('json')->fail('您选择的的订单存在用户未删除的订单');
            if ($this->services->batchUpdate($ids, ['is_system_del' => 1])) return app('json')->success('删除成功');
        }
        if ($all == 1) $ids = [];
        $type = 6;// 订单删除
        $where['status'] = -4;
    ...
    public function update_refund(Request $request, StoreOrderRefundServices $services, StoreOrderCreateServices $storeOrderCreateServices, StoreOrderCartInfoServices $storeOrderCartInfoServices, $id)
    {
        ...
        $cacheKey = 'refund_' . $adminId . '_' . $id;
        if ($services->cacheHander()->has($cacheKey)) {
            return app('json')->fail('请勿重复操作!');
        }
        $services->cacheTag()->set($cacheKey, 1, 1);
        ...
        if ($type == 2) {
            ...
            $refundOrderData['channel'] = $order['channel'];
...
//                $services->storeProductOrderRefundYFasle((int)$id, $refund_price);
    ...
    {
        $adminId = (int)$request->adminId();
        $cacheKey = 'refund_open_' . $adminId . '_' . $id;
        if ($services->cacheHander()->has($cacheKey)) {
            return app('json')->fail('请勿重复操作!');
        }
        $services->cacheTag()->set($cacheKey, 1, 1);
