...
class OrderServices
{
    ...
    public function upload(int $oid): bool
    {
        ...
        for ($i = $num; $i >= 0; $i--) {
            if ($i <= 0) {
                Log::error(
                    json_encode(['msg' => 'ERP订单上传失败,调用均为异常', 'oid' => $oid], JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT)
                );
                return false;
            }
    ...
    public function cancelCallback(array $cancel): bool
    {
        try {
            ...
            Log::error(
                json_encode(['msg' => '订单取消失败,原因:' . $e->getMessage(), 'data' => $cancel], JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT)
            );
        }
        return true;
    }
...
//            }
    ...
    public function refundOrderUpload(array $refundIds, string $shopStatus = 'WAIT_SELLER_CONFIRM_GOODS')
    {
        try {
            ...
            Log::error(
                json_encode(['msg' => '售后单上传失败，原因:' . $e->getMessage(), 'data' => ['refundIds' => $refundIds, 'shopStatus' => $shopStatus]], JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT)
            );
        }
    }
