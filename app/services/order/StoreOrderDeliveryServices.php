...
//        return true;
    ...
    public function orderDeliverGoods(int $id, array $data, $orderInfo, $storeTitle)
    {
        ...
        if ($data['express_record_type'] == 2) {//电子面单
            ...
            $expData['customParam'] = json_encode([
                'product_name' => $orderInfoServices->getCarIdByProductTitle((int)$orderInfo['id'], true)
            ]);
...
//            }
        ...
        if ($data['express_record_type'] != 3) {
            ...
            $this->transaction(function () use ($id, $data) {
                ...
                //记录主订单状态
                OrderStatusJob::dispatch([$id, 'delivery_goods', ['change_message' => '已发货 快递公司：' . $data['delivery_name'] . ' 快递单号：' . $data['delivery_id'], 'change_manager_id' => request()->adminId(), 'change_manager_type' => 'admin']]);
            });
        } else {
    ...
    public function orderDelivery(int $id, array $data, $orderInfo = [])
    {
        ...
        $this->transaction(function () use ($id, $data, $delivery_type, $station_type) {
            ...
            //记录主订单状态
            OrderStatusJob::dispatch([$id, $delivery_type == 1 ? 'delivery' : 'city_delivery', ['change_message' => $message, 'change_manager_type' => 'system']]);
    ...
    public function orderVirtualDelivery(int $id, array $data)
    {
        ...
        return $this->transaction(function () use ($id, $data) {
            ...
            //记录主订单状态
            OrderStatusJob::dispatch([$id, 'delivery_fictitious', ['change_message' => '已虚拟发货', 'change_manager_id' => request()->adminId(), 'change_manager_type' => 'admin']]);
            return true;
        });
    }
    ...
    public function updateDistribution(int $id, array $data)
    {
        ...
        $this->dao->update($id, $data);
        OrderStatusJob::dispatch([$id, 'distribution', ['change_message' => '修改发货信息为' . $data['delivery_name'] . '号' . $data['delivery_id'], 'change_manager_id' => request()->adminId(), 'change_manager_type' => 'admin']]);
        return true;
    }
