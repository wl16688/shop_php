...
//        $locationOption->setRegion($region);
    ...
    public function doCancel(int $id, $deliveryOrder, $deduct_fee, $mark)
    {
        ...
        //记录订单状态
        OrderStatusJob::dispatch([$deliveryOrder['oid'], 'city_delivery_cancel', ['change_message' => '同城配送取消', 'change_manager_id' => request()->adminId(), 'change_manager_type' => 'admin']]);
        return true;
    }
    ...
    public function notifyAfter($status, $reason, $deliveryOrder, $data, $deductFee)
    {
        ...
        if ($orderData['status'] != $status && !$statusServices->count(['oid' => $oid, 'change_type' => $change_type])) {
            ...
            //记录原订单状态
            OrderStatusJob::dispatch([$oid, $change_type, ['change_message' => $message, 'change_manager_type' => 'system']]);
            switch ($status) {
                case 2:
