...
//        if ($paytype == PayServices::ALIPAY_PAY && isset($other['trade_no'])) {
...
//        }
        ...
        //支付成功后发送消息
        $orderInfo = $this->dao->get($orderInfo['id']);
    ...
    public function getMemberRecord(array $where, int $limit = 0)
    {
        ...
        if ($list) {
            ...
            foreach ($list as &$v) {
                ...
                // 如果$v['member_type']不是数字
                if (!is_numeric($v['member_type'])) {
                    if ($v['member_type'] == 'admin') $v['member_type'] = '赠送';
                    if ($v['member_type'] == 'card') $v['member_type'] = '卡密兑换';
                } else {
                    $v['member_type'] = $shipInfo[$v['member_type']]['title'] ?? '';
                }
                $v['pay_time'] = $v['pay_time'] ? date('Y-m-d H:i:s', $v['pay_time']) : '';
                $v['add_time'] = date('Y-m-d H:i:s', $v['add_time']);
                switch ($v['pay_type']) {
