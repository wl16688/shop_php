...
class UserBrokerageServices extends BaseServices
{
     ...
     * @return float
     */
    public function getUserBillBrokerageSum(int $uid, array $type = ['self_brokerage', 'one_brokerage', 'two_brokerage', 'brokerage_user', 'staff_brokerage', 'agent_brokerage', 'division_brokerage'], $time = '')
    {
        $where = ['uid' => $uid];
...
//            $this->dao->update($where, ['frozen_time' => 0]);
    ...
    public function spreadOrderList(int $uid, array $data, int $page = 0, int $limit = 0)
    {
        ...
        $where = ['pid' => 0, 'paid' => 1, 'refund_status' => [0, 3], 'is_del' => 0, 'is_system_del' => 0, 'division_spread' => $uid];
