...
//        $field[] = Form::number('valid_date', '有效时间(天)', $vipinfo['valid_date'] ?? 0)->min(0)->col(12);
        ...
        $field[] = Form::frameImage('icon', '卡片(656*300)', Url::buildUrl('admin/widget.images/index', array('fodder' => 'icon')), $vipinfo['icon'] ?? '')->icon('ios-add')->width('960px')->height('505px')->modal(['footer-hide' => true])->appendValidate(Iview::validateStr()->required()->message('请选择图标'));
        $field[] = Form::frameImage('image', '卡片背景(750*990)', Url::buildUrl('admin/widget.images/index', array('fodder' => 'image')), $vipinfo['image'] ?? '')->icon('ios-add')->width('960px')->height('505px')->modal(['footer-hide' => true])->appendValidate(Iview::validateStr()->required()->message('请选择背景'));
        $field[] = Form::color('color', '字体颜色', $vipinfo['color'] ?? '#000000')->required('请选择字体颜色');
    ...
    public function detection(int $uid)
    {
        ...
        if (!$user['level_status'] && sys_config('level_activate_status') == 0) {
            $userServices->update($uid, ['level_status' => 1]);
