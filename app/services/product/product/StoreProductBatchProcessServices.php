...
//        'custom_form' => [],//自定义留言
    ...
    public function batchProcess(int $type, array $ids, array $input_data, bool $is_all = false, array $where = [])
    {
        ...
        switch ($type) {
            ...
            case 8://运费设置
                ...
                break;
            case 9://商品品牌 不选默认清空标签
                $data['brand_com'] = $input_data['brand_id'] ? implode(',', $input_data['brand_id']) : '';
                $data['brand_id'] = $input_data['brand_id'] ? end($input_data['brand_id']) : 0;
                break;
            case 10://商品佣金设置
                $data['brokerage'] = $input_data['brokerage'] ?? 0;
                $data['brokerage_two'] = $input_data['brokerage_two'] ?? 0;
                break;
            default:
    ...
    public function runBatch(array $ids, array $data, int $type = 2)
    {
        ...
        switch ($type) {
            ...
            case 10://商品佣金
                foreach ($ids as $id) {
                    ProductBrokerageJob::dispatch([$id, $data['brokerage'], $data['brokerage_two']]);
                }
                break;
            default://
                $this->dao->batchUpdate($ids, $data, 'id');
