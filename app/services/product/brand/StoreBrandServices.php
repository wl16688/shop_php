...
//        $res = $res && $this->dao->update($id, ['is_show' => $is_show], 'pid');
     ...
     * @date 2025/5/28
     */
    public function getBrandId($product_brand)
    {
        $brand_id = $this->dao->value(['brand_name' => $product_brand], 'id');
        if (!$brand_id) {
            $brand_id = $this->dao->save(['brand_name' => $product_brand, 'add_time' => time()])['id'];
        }
        return [$brand_id];
    }
}
