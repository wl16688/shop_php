...
class DeliveryServiceServices extends BaseServices
{
    ...
    /**
     * @var DeliveryServiceDao
     */
    #[Inject]
    protected DeliveryServiceDao $dao;
...
//            $field[] = $this->builder->frameImage('avatar', '用户头像', $this->url('admin/widget.images/index', array('fodder' => 'avatar')))->icon('ios-add')->width('50%')->height('396px');
        ...
        $field[] = $this->builder->radio('status', '配送员状态', $formData['status'] ?? 1)->options([['value' => 1, 'label' => '开启'], ['value' => 0, 'label' => '关闭']]);
        return $field;
    }
    ...
    public function createStoreDeliveryForm()
    {
        ...
        $field[] = $this->builder->radio('status', '配送员状态', 1)->options([['value' => 1, 'label' => '开启'], ['value' => 0, 'label' => '关闭']]);
