...
class SystemStoreServices extends BaseServices
{
    ...
    public function storeAdminAccountForm(int $id)
    {
        ...
        return create_form('自提点重置账号密码', $field, $this->url('/store/store/reset_admin/' . $id));
    }
    ...
    public function getStoreDispose(int $id, string $felid = '')
    {
        if ($felid) {
            ...
            if ($storeInfo) {
                ...
                $storeInfo['address2'] = $storeInfo['address2'] ? explode(',', $storeInfo['address2']) : [];
                return $storeInfo;
            }
            return false;
        }
