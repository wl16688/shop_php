...
class QueueServices extends BaseServices
{
    ...
    public array $queue_redis_key = [
        ...
        9 => 5,
        10 => 6,
        11 => "ProductRecycle-ADMIN",
        12 => "ProductDeleteBatch-ADMIN",
    ];
    ...
    public function setQueueData(array $where = [], $field = "*", array $data = [], int $type = 1, $other = false)
    {
        ...
        $source = 'admin';
        if (in_array($type, [1, 2, 3, 4, 5, 6, 11, 12])) {
...
//                    $is_activity = $storeproductServices->checkActivity($v);
     ...
     * @return void
     * User: liusl
     * DateTime: 2025/5/22 11:37
     */
    public function productRecycle($type)
    {
        ...
        if ($pids) {
            $chunkPids = array_chunk($pids, 1000, true);
            $res = app()->make(StoreProductServices::class)->delAll($pids);
            foreach ($chunkPids as $v) {
                if ($res) {
                    $this->doSuccessSremRedis($v, $redisKey, $type);
                } else {
     ...
     * @throws \think\db\exception\ModelNotFoundException
     * User: liusl
     * DateTime: 2025/5/23 10:22
     */
    public function productDeleteBatch($type)
    {
        ...
        if ($pids) {
            $chunkPids = array_chunk($pids, 1000, true);
            $res = app()->make(SystemClearServices::class)->recycleProduct($pids);
            foreach ($chunkPids as $v) {
                if ($res) {
                    $this->doSuccessSremRedis($v, $redisKey, $type);
                } else {
