...
class UserPointServices extends BaseServices
{
    /**
    ...
    public function recordRemark($id, $mark)
    {
        ...
        if ($mark === '') throw new AdminException('请填写备注');
    ...
    public function getTrend($where)
    {
        ...
        if (count($time) != 2) throw new AdminException('数据错误');
    ...
    public function getChannel($where)
    {
        ...
        $data = ['gain', 'gain', 'system_add', 'sign', 'lottery_add'];
        $bing_data = [];
        foreach ($data as $key => $item) {
            $bing_data[] = [
                'name' => $bing_xdata[$key],
    ...
    public function getType($where)
    {
        ...
        $color = ['#64a1f4', '#3edeb5', '#70869f', '#ffc653', '#fc7d6a'];
        $data = ['deduction', 'lottery_use', 'system_sub', 'pay_product_integral_back', 'storeIntegral_use'];
        $bing_data = [];
        foreach ($data as $key => $item) {
            $bing_data[] = [
                'name' => $bing_xdata[$key],
                'value' => $this->dao->sum(['type' => $item, 'category' => 'integral', 'time' => $where['time']], 'number', true),
                'itemStyle' => ['color' => $color[$key]]
            ];
        }
