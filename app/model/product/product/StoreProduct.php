...
//                    }
    ...
    public function searchisGeneralProductAttr($query, $value)
    {
        if ('' !== $value) $query->where('is_general_product', $value);
    }

    public function searchProductClearAttr($query, $value)
    {
        if ($value !== '') {
            if ($value == 'is_general_product') {
                $query->where('is_general_product', 1);
            }
            if ($value == 'is_channel_product') {
                $query->where('is_channel_product', 1);
            }
            if ($value == 'is_vip_product') {
                $query->where('is_vip_product', 1);
            }
        }
    }
