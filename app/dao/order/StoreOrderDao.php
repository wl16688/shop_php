...
class StoreOrderDao extends BaseDao
{
     ...
     * @date 2025/5/10
     */
    public function getOneData(array $where = [], string $field = '*', string $order = '', array $with = [])
    {
        return $this->getModel()->where($where)->with($with)->field($field)->order($order)->find();
    }
     ...
     * @date 2025/5/10
     */
    public function getManyData(array $where = [], string $field = '*', string $order = '', array $with = [])
    {
        return $this->getModel()->where($where)->with($with)->field($field)->order($order)->select();
    }
    ...
    public function getUserOrderDetail(string $key, int $uid = 0, array $with = [])
    {
        ...
        if ($uid) $where = $where + ['uid|receive_gift_uid' => $uid];
     ...
     * @return mixed
     */
    public function getProductTrend($time, $timeType, $field, $str, $orderStatus = '',$supplier_id = '')
    {
