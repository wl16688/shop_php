...
//                '(select count(*) from (SELECT COUNT(`pay_uid`) as p,product_id FROM `' . $prefix . 'store_product_log` WHERE `product_id` = `' . $prefix . 'store_product_log`.`product_id` AND `type` = \'pay\' GROUP BY `pay_uid` HAVING p>1) u WHERE `product_id` = `' . $prefix . 'store_product_log`.`product_id`) as aaaa',
     ...
     */
    public function getProduct(array $where, int $page = 0, int $limit = 0)
    {
        $model = $this->getModel()->alias('L')
            ->join('StoreProduct S', 'S.id = L.product_id')
            ->where('S.is_show', 1)->where('S.is_del', 0);
        if (isset($where['keyword']) && $where['keyword'] != '') {
            $model->whereLike('store_name|keyword', '%' . $where['keyword'] . '%');
        }
        if (isset($where['type']) && $where['type'] != '') {
            $model = $model->where('L.type', $where['type']);
        }
        if (isset($where['uid']) && $where['uid'] != '') {
            $model = $model->where('L.uid', $where['uid']);
        }
        return $model->when($page != 0 && $limit != 0, function ($query) use ($page, $limit) {
            $query->page($page, $limit);
        })->group('product_id')->field(['L.product_id', 'S.image', 'S.store_name', 'S.price'])->order('L.add_time desc')->select()
            ->toArray();
