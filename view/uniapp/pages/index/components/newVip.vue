	...
	<view :style="[newVipWrapStyle]" v-if="(couponList.length || productList.length || newcomer_integral) && !dataConfig.isHide">
		<view class="newVip2" :style="[newVip2Background,newVipBorderRadius]" v-if="dataConfig.styleConfig.tabVal">
			...
			<view class="wrapper">
				...
				<view class="product" v-if="productList.length && dataConfig.checkboxInfo.type.includes(2)">
					...
					<view class="content">
						<scroll-view scroll-x="true">
							<view class="px-20 grid-column-4 grid-gap-16rpx">
								<view class="w-full" v-for="(item,index) in productList" :key="item.id" @click="goDetail(item)">
									...
									<view class="name w-140 line1">{{ item.store_name }}</view>
								</view>
							</view>
						</scroll-view>
					</view>
		...
		<view class="newVip" v-else>
			...
			<view class="product" v-if="productList.length && dataConfig.checkboxInfo.type.includes(2)">
				...
				<view class="content">
					<scroll-view scroll-x="true">
						<view class="grid-column-4 grid-gap-16rpx">
							<view class="w-full" v-for="(item,index) in productList" :key="item.id" @click="goDetail(item)">
								<easy-loadimage :image-src="item.image" width="100%" height="144rpx" borderRadius="12rpx"></easy-loadimage>
								<view class="w-140 line1">{{ item.store_name }}</view>
