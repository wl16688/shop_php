<template>
	<view :style="[signWrapStyle]" v-if="!dataConfig.isHide">
		<view class="signIn" :style="[signStyle]">
			<view v-if="dataConfig.styleConfig.tabVal" class="text-wrap acea-row row-middle">
				...
				<view class="text">
					...
					<view>连续签到{{nextContinuousSignRewardList.days||0}}天，额外活动{{nextContinuousSignRewardList.point}}积分</view>
				</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getSign() {
				getSign().then(res => {
					if(res.data.nextContinuousSignRewardList.length){
