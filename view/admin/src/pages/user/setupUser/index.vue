<template>
  <div>
    <Card :bordered="false" dis-hover class="ivu-mt">
      ...
      <!-- ---------------------------------基础信息-------------------------------- -->
      <div v-show="tabVal == 'basic'">
        <Alert type="warning" show-icon v-if="register_notice">{{
          register_notice
        }}</Alert>
          ...
          <Row :gutter="24" type="flex">
            ...
            <Col span="24" class="mt10">
              <FormItem label="用户默认头像：">
                ...
                <div class="uploadPictrue" @click="modalPicTap('单选')" v-else>
            ...
            <Col span="14" class="mt10">
              <FormItem label="用户信息设置：">
                ...
                >
                  ...
                  <template slot-scope="{ row, index }" slot="is_only">
                    <span>{{ row.is_only ? '是' : '否' }}</span>
                  </template>

                  <template slot-scope="{ row, index }" slot="action">
                    <a @click="delInfo(index)" v-if="!listOne[index].param"
                      >删除</a
                    >
                  </template>
                </Table>
                ...
                >
                  ...
                  >
                    ...
                    <template slot-scope="{ row, index }" slot="status">
                      <a @click="delCoupon(index, indexw, item.giveCoupon)">删除</a>
                    </template>
                  </Table>
                  <div
                    class="add-coupon"
      ...
      <!-- ---------------------------------会员等级-------------------------------- -->
      <div v-show="tabVal == 'level'">
        <Form :model="vipForm" :label-width="140">
          <Row :gutter="24" type="flex">
            <Col span="24">
              <div class="basics">等级设置</div>
            </Col>
            <Col span="24" class="mt10">
              <FormItem label="会员等级启用：">
                ...
                <div class="upload-text">开启会员等级后，可以获得经验值</div>
              </FormItem>
            </Col>
            <Col span="24" v-if="vipForm.member_func_status === 1">
            ...
            >
              <FormItem label="会员卡信息：">
                ...
                >
                  ...
                  <template slot-scope="{ row, index }" slot="required">
                    <Checkbox
                      v-model="listVip[index].required"
                      @on-change="tapCheckbox"
                    ></Checkbox>
                  </template>
                  <template slot-scope="{ row, index }" slot="action">
              ...
              <FormItem label="赠送优惠券：">
                ...
                >
                  ...
                  <Table
                    border
                    :columns="columns1"
                    :data="vipForm.level_give_coupon"
                    ref="table"
                    class="table"
                    width="700"
                    v-if="vipForm.level_give_coupon.length && vipForm.level_coupon_status == 1">
                    <template slot-scope="{ row }" slot="coupon_price">
                      <span v-if="row.coupon_type == 1">
                        {{ row.coupon_price }}元
                      </span>
                    ...
                    <template slot-scope="{ row, index }" slot="status">
                      <a @click="delCoupon(index, indexw, vipForm.level_give_coupon)">删除</a>
                    </template>
                  </Table>
                </div>
              </FormItem>
    ...
    >
      ...
      >
        ...
        <FormItem label="信息格式 ：" prop="format">
          <Select v-model="formItem.format" @on-change="formatChange" style="width: 300px">
            <Option
        ...
        <FormItem label="单选项 ：" prop="singlearr"  v-if="formItem.format === 'radio'">
          <div class="arrbox">
            <Tag
        ...
        <FormItem v-if="showOnly">
          <Checkbox v-model="formItem.is_only" :true-value="1" :false-value="0">
            <span class="pl-8">用户填写信息不可重复</span>
          </Checkbox>
        </FormItem>
      </Form>
...
export default {
  ...
  watch: {
    "$route.path": {
      handler(newPath) {
        if (newPath == "/admin/user/userSet") {
          this.tabVal = "basic"; // 重置 tabVal
        } else if (newPath == "/admin/user/levelSet") {
          this.tabVal = "level";
        } else if (newPath == "/admin/user/registerSet") {
          this.tabVal = "register";
        } else if (newPath == "/admin/user/svipSet") {
          this.tabVal = "svip";
        } else {
          this.tabVal = "member_gift";
        }
        this.settingUser();
        // 当 query 参数变化时执行
        // this.tabVal = newQuery.type;
      },
      deep: true,
  ...
  methods: {
    ...
    settingUser() {
      ...
      settingUser(this.tabVal)
        .then((res) => {
          if (this.tabVal === "basic") {
            this.authorizedPicture = res.data.h5_avatar;
            this.listOne = res.data.user_extend_info;
            this.store_user_mobile = res.data.store_user_mobile;
            this.register_notice = res.data.register_notice;
          }
        ...
        .catch((err) => {
          this.$Message.error(err.msg || "获取用户配置失败");
        });
    },
      ...
      switch (val) {
        case "basic":
          let data = {
            ...
            store_user_mobile: this.store_user_mobile,
          };
          setUser(val, data)
            .then((res) => {
    ...
    getCouponList(data) {
      ...
      if (this.tabVal === "register") {
        ...
        this.vipCopon = uni;
        this.vipForm.level_give_coupon = this.vipCopon;
      }
    },

    // 删除优惠券
    delCoupon(index, indexw, data) {
      data.splice(index, 1);
      // if (this.tabVal === "level") {
    ...
    formatChange(val){
      if(val == 'date' || val == 'radio'){
        this.showOnly = false;
        this.formItem.is_only = 0;
      }else{
        this.showOnly = true;
      }
    }
