  ...
  <div class="user-info">
    <div class="section">
      ...
      <div class="section-bd">
        ...
        <div class="item">
          <div>性别：</div>
          <div v-if="psInfo.sex" class="value">
            {{ psInfo.sex == 1 ? "男" : "女" }}
          </div>
        ...
        <div class="item">
          <div>用户地址：</div>
          <div class="value">
            {{ `${psInfo.provincials}${psInfo.addres}` || "-" }}
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="section">
      ...
      <div class="flex-y-center mt-14 fs-13 text--w111-606266">
        <div>手动标签：</div>
        <div class="pr-14" v-for="(item, index) in labelTypeOne" :key="index">{{ item.label_name }}</div>
      </div>
      <div class="flex-y-center mt-14 fs-13 text--w111-606266">
        <div>自动标签：</div>
        <div class="pr-14" v-for="(item, index) in labelTypeTwo" :key="index">{{ item.label_name }}</div>
      </div>
    </div>
      ...
      <div class="section-bd">
        <template v-for="item in psInfo.extend_info">
          <div v-if="item.value" :key="item.sort" class="item">
            <div>{{ item.info }}：</div>
            <div class="value" v-if="item.param == 'sex'">
              {{ item.singlearr[item.value] }}
            </div>
...
export default {
  ...
  computed: {
    ...
    labelTypeOne() {
      return this.psInfo.label_id.filter((item) => item.label_type === 1);
    },
    labelTypeTwo() {
      return this.psInfo.label_id.filter((item) => item.label_type === 2);
    },
  },
};
</script>
