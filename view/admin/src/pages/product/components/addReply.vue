<template>
  ...
  >
    <Form :model="formData" :label-width="125">
      <template v-if="aiStatus == 0 || aiStatus == 1">
        ...
        <FormItem v-if="goods.id" label="商品规格" required>
          <div class="upload-box" @click="callAttr">
            <el-tooltip
              ...
              placement="right"
            >
              <img :src="attr.image" class="image" />
            </el-tooltip>
            <Icon v-else type="ios-add" />
          </div>
        </FormItem>
      ...
      <template v-if="aiStatus !== 1">
        ...
        <FormItem label="评价图片" v-if="aiStatus == 0">
    ...
    <template slot="footer">
      ...
      <Button :loading="loading" type="primary" @click="onOk">{{ aiStatus == 1 ? 'AI生成' : '确定' }}</Button>
    </template>
  </Modal>
</template>
...
export default {
  props: {
    ...
    aiStatus: {
      type: Number,
      default: 0, // 0 正常添加 1 是AI添加 2 是AI编辑
    },
    replyEidtData: {
      type: Object,
      default() {
  ...
  watch: {
    ...
    replyEidtData(value) {
      if (value) {
        this.formData = value;
        this.product_score = value.product_score;
        this.service_score = value.service_score;
        this.delivery_score = value.delivery_score;
        this.add_time = value.add_time;
        this.reply_score = value.reply_score;
      }
    },
    aiStatus(value) {
      if (value == 0) {
  ...
  methods: {
    ...
    // 生成
    async generateFromPopover(apiParams) {
      this.loading = true;
      try {
