  ...
  >
    ...
    >
      <Row type="flex" v-show="currentTab === '1'">
        ...
        <Col span="24" v-if="formValidate.video_open">
          <FormItem label="" prop="video_link">
            ...
            >
            <Upload
              v-if="upload_type === '1' && !formValidate.video_link"
      ...
      <Row :gutter="24" type="flex" v-show="currentTab === '2'">
        ...
        <Col
          span="24"
          v-if="formValidate.spec_type === 1 && formValidate.product_type != 4"
          class="noForm"
        >
          <Col span="24">
          ...
          <Col span="24" v-if="createBnt">
            <FormItem>
              <Button type="primary" icon="md-add" @click="addBtn" class="mr15"
                >添加新规格</Button
              >
            ...
            <Col span="24">
              <FormItem label="商品属性：" class="labeltop">
                ...
                >
                  ...
                  >
                    <div
                      :class="manyFormValidate[index].select ? 'selectOn' : ''"
                    >
        ...
        <!-- 单规格表格-->
        <div
          v-if="formValidate.spec_type === 0 || formValidate.product_type == 4"
          style="width: 100%"
        >
          <Col span="24" v-if="formValidate.product_type != 4">
            ...
            <FormItem label="" prop="freight">
              <div class="acea-row row-middle">
                <DatePicker
                  ...
                  v-width="'50%'"
                  :value="oneFormValidate[0].section_time"
                  v-model="oneFormValidate[0].section_time"
                ></DatePicker>
              </div>
            </FormItem>
          </Col>
          ...
          <div class="labelInput acea-row row-between-wrapper" @click="openLabel">
            <div>
              <div v-if="dataLabel.length">
                <Tag closable v-for="(item, index) in dataLabel" :key="index" @on-close="closeLabel(item)">{{ item.label_name }}
                </Tag>
              </div>
              <Icon type="ios-arrow-down" />
            </div>
          </div>
          <div class="fs-12 text--w111-999 mt10">用户购买该商品后，自动为用户关联所选标签</div>
        </FormItem>
        </Col>
        <Col span="24">
        ...
        <Col span="24">
          <FormItem label="服务保障：">
            <CheckboxGroup v-model="formValidate.ensure_id" class="checkAlls">
              <Checkbox :label="item.id" v-for="(item, index) in ensureData" :key="index">{{
                item.name
              }}</Checkbox>
            </CheckboxGroup>
          </FormItem>
    ...
    handleSubmit(name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          ...
          if (
            this.formValidate.product_type == 4 &&
            !this.oneFormValidate[0].write_times
          ) {
            return this.$Message.warning("规格库存-核销次数必须大于0");
          }
          if (
            this.formValidate.product_type == 4 &&
            this.oneFormValidate[0].write_valid == 2 &&
            !this.oneFormValidate[0].days
          ) {
            return this.$Message.warning("规格库存-有效天数必须大于0");
          }
          if (
            this.formValidate.product_type == 4 &&
            this.oneFormValidate[0].write_valid == 3 &&
            !this.oneFormValidate[0].section_time.length
          ) {
