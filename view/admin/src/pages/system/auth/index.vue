      ...
      <span class="prompt">
        ...
        >
          ...
          <div class="exampleImg" slot="content">
            <img :src="`${baseURLS}/statics/system/mobileAuth.png`" alt="" />
          </div>
        </Poptip>
      </span>
    </Card>
    ...
    >
      ...
      >
        <template slot-scope="{ row }" slot="value">
          <div
            v-if="row.value == true"
            class="iconfont iconwancheng text-wlll-2d8cf0 fs-18"
          ></div>
          <div
            v-else-if="row.value == false"
            class="iconfont iconguanbi text-wlll-f5222d fs-18"
          ></div>
  ...
  methods: {
    systemInfo() {
      getSystemInfo()
        .then((res) => {
          let data = res.data;
          const keys = ["server", "environment", "permissions", "process"];
          keys.forEach((key, index) => {
            this.dataList[index].data = data[key];
          });
        })
        .catch((err) => {
