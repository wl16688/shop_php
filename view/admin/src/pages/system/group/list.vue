    ...
    <div class="i-layout-page-header">
      <PageHeader class="product_tabs" hidden-breadcrumb>
        <div slot="title" class="acea-row row-middle">
          <router-link
            :to="{ path: '/admin/system/config/system_group/index' }"
          >
    ...
    <Card :bordered="false" dis-hover class="ivu-mt">
      ...
      <Card :bordered="false" dis-hover :padding="0">
        <Form
          ...
          @submit.native.prevent
        >
          <Button type="primary" @click="groupAdd('添加数据')" class="mr20"
            >添加数据</Button
          >
        </Form>
        <Table
...
export default {
  ...
  methods: {
    ...
    edit(row) {
      ...
      this.$modalForm(
        groupDataEditApi(data, this.getUrl(`/${row.id}/edit`))
      ).then(() => this.getList());
    },
    // 删除
    del(row, tit, num) {
