      ...
      <div class="new_card_pd">
        ...
        >
          ...
          <FormItem label="搜索" label-for="name">
            ...
            />
            <Button type="primary" @click="search()" class="btn-add"
              >查询</Button
            >
          </FormItem>
        </Form>
    ...
    <Card :bordered="false" dis-hover class="ivu-mt">
      ...
      >
        <template slot-scope="{ row }" slot="send_type">
          <Tag color="green" size="medium" v-if="row.send_type == 1"
            >已发送</Tag
          >
          <Tag color="red" size="medium" v-if="row.send_type == 0">未发送</Tag>
          <Tooltip max-width="200" placement="bottom">
            <Tag color="orange" size="medium" v-if="row.send_type == -1"
              >未发送成功</Tag
            >
        ...
        <template slot-scope="{ row }" slot="template_type">
          <Tag color="cyan" size="medium" v-if="row.template_type"
            >定时发送</Tag
          >
          <Tag color="blue" size="medium" v-else>立即发送</Tag>
        </template>
        <template slot-scope="{ row, index }" slot="action">
          <a @click="sendMessage(row, index)" :disabled="row.send_type == 1"
            >提醒发送</a
          >
...
export default {
  ...
  methods: {
    getList() {
      this.loading = true;
      getGroupTemplateChatList(this.tableFrom)
        .then((res) => {
