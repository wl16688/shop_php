      ...
      >
        ...
        <template slot-scope="{ row }" slot="status">
          <Tag
            color="default"
            size="large"
            v-show="[103, 104, 107].indexOf(row.live_status) != -1"
            >已结束</Tag
          >
          <Tag
            color="orange"
            size="large"
            v-show="[102].indexOf(row.live_status) != -1"
            >未开始</Tag
          >
          <Tag
            color="green"
            size="large"
            v-show="[101, 105, 106].indexOf(row.live_status) != -1"
            >直播中</Tag
          >
        </template>
...
export default {
  ...
  methods: {
    ...
    menusAdd() {
      this.$router.push({
        path: "/admin/content/live/add_live_room",
      });
    },
    // 分页
    pageChange(index) {
