    ...
    <Card :bordered="false" dis-hover class="ivu-mt" :padding="0">
      <div class="new_card_pd">
        ...
        >
          ...
          <FormItem>
            <Button type="primary" @click="orderSearch()" class="btn-add"
              >查询</Button
            >
    ...
    <Card :bordered="false" dis-hover class="ivu-mt">
      ...
      >
        ...
        <template slot-scope="{ row, index }" slot="is_invoice">
          <Tag color="orange" size="medium" v-if="row.is_invoice === 0"
            >未开票</Tag
          >
          <Tag color="green" size="medium" v-else-if="row.is_invoice === 1"
            >已开票</Tag
          >
        ...
        <template slot-scope="{ row, index }" slot="status">
          ...
          <template v-else>
            <Tag color="green" size="medium" v-if="row.status === 0"
              >未发货</Tag
            >
            <Tag color="blue" size="medium" v-else-if="row.status === 1"
              >待收货</Tag
            >
            <Tag color="orange" size="medium" v-else-if="row.status === 2"
              >待评价</Tag
            >
            <Tag color="volcano" size="medium" v-else-if="row.status === 3"
              >已完成</Tag
            >
          </template>
        </template>
        ...
        <template slot-scope="{ row, index }" slot="action">
          <a
            :disabled="row.refund_status !== 0 || row.is_invoice == -1"
            @click="edit(row)"
            >编辑</a
          >
      ...
      >
        ...
        >
          <div class="list">
            ...
            <Row class="row">
              ...
              <Col span="12" v-if="invoiceDetails.is_invoice === 1"
                >发票金额: ￥{{ invoiceDetails.pay_price }}</Col
              >
            </Row>
          </div>
          <div class="list">
        ...
        <FormItem label="发票金额：" v-if="formInline.is_invoice === 1">
          <Input
            @input="inputEnter"
            @blur="inputBlur"
            v-model="formInline.invoice_amount"
            placeholder="请输入发票金额"
          ></Input>
        ...
        <Button type="primary" @click="handleSubmit()" class="ok-btn"
          >确定</Button
        >
      </Form>
    </Modal>
...
export default {
  ...
  methods: {
    ...
    edit(row) {
      ...
      this.formInline.invoice_amount =
        row.invoice_amount == "0.00" ? row.pay_price : row.invoice_amount;
    },
    // 订单列表
    getList() {
      this.loading = true;
    ...
    inputEnter() {
      ...
      if (this.formInline.pay_price / 1 > 256) {
        this.formInline.pay_price = this.formInline.pay_price + "";
        this.formInline.pay_price = this.formInline.pay_price.slice(
          0,
          this.formInline.pay_price.length - 1
        );
      }
    },
    inputBlur() {
