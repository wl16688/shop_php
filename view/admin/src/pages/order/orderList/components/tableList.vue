  ...
  <div>
    ...
    <div class="acea-row row-between">
      ...
      <div>
        ...
        <Button class="mr10" @click="exports(0)"> 导出订单 </Button>
        <Button
          v-auth="['order-write']"
          class="mr10 greens"
          size="default"
    ...
    >
      ...
      <vxe-column field="pink_name" title="订单类型" min-width="120">
        <template v-slot="{ row }">
          <span>{{ removeBrackets(row.pink_name) }}</span>
        </template>
      </vxe-column>
      ...
      <vxe-column field="info" title="商品信息" min-width="330">
        <template v-slot="{ row }">
          <Tooltip :transfer="true" theme="dark" max-width="800" :delay="600">
            <div class="tabBox" v-for="(val, i, index) in row._info" :key="i">
            ...
            <div slot="content">
              <div class="mb-10 flex" v-for="(val, i) in row._info" :key="i">
                <span
                  ><span class="font-color-red" v-if="val.cart_info.is_gift"
                    >赠品</span
                  ><span>{{ val.cart_info.productInfo.store_name }}</span></span
                >
      ...
      >
        <template v-slot="{ row }">
          ...
          <template
            ...
            "
          >
            <a :disabled="openErp" @click="sendOrder(row)">发送货</a>
    ...
    >
      <Row type="flex">
        <Col span="4">
          <div style="line-height: 32px; text-align: right">批量发货：</div>
        </Col>
        <Col span="20">
          <Upload
            ref="upload"
          ...
          >
            <Button icon="ios-cloud-upload-outline">上传发货单</Button>
          </Upload>
        </Col>
        <Col span="4">
          <div style="line-height: 32px; text-align: right">下载文件：</div>
        </Col>
        <Col span="20">
          <Button
          ...
          >
            <template slot-scope="{ row }" slot="product">
              ...
              >
                ...
                <template #content>
                  <div class="title">
                    商品名称:{{ row.productInfo.store_name }}
                  </div>
                  <div class="title">
                    商品规格：{{ row.productInfo.attrInfo.suk }}
                  </div>
                </template>
              </Tooltip>
            </template>
...
export default {
  ...
  methods: {
    ...
    goodsDetails(val) {
      console.log(val);
      this.productId = val.cart_info.product_id;
        ...
        for (let i = 0; i < excelData.page + 1; i++) {
          ...
          if (lebData.export.length) {
            ...
            for (let j = 0; j < data.length; j++) {
              // 安全处理可能不存在的字段
              let goodsList = data[j]["goods_name"]
                ? data[j]["goods_name"].split("\n")
                : [""];
              let attr = data[j]["attr"] ? data[j]["attr"].split("\n") : [""];
              let product_id = data[j]["product_id"]
                ? data[j]["product_id"].split("\n")
                : [""];
              let cart_num = data[j]["cart_num"]
                ? data[j]["cart_num"].split("\n")
                : [""];
              ...
              goodsList = padArray(goodsList, maxLength);
              attr = padArray(attr, maxLength);
              product_id = padArray(product_id, maxLength);
              cart_num = padArray(cart_num, maxLength);

              for (let k = 0; k < maxLength; k++) {
                result.push({ ...data[j] });
                result[result.length - 1]["goods_name"] = goodsList[k] || "";
                result[result.length - 1]["product_id"] = product_id[k] || "";
                result[result.length - 1]["attr"] = attr[k] || "";
                result[result.length - 1]["cart_num"] = cart_num[k] || "";
                if (!k) {
                  continue;
                }
                for (const key in result[result.length - 1]) {
                  if (
                    ["goods_name", "product_id", "attr", "cart_num"].includes(
                      key
                    )
                  ) {
                    continue;
