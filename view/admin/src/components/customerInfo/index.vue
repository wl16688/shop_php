<template>
  <!-- 选择用户 -->
  <div>
    <Form ref="formValidate" inline :model="formValidate" :label-width="90" label-position="right"
      @submit.native.prevent>
      <FormItem label="注册日期：">
        <DatePicker :editable="false" @on-change="onchangeTime" :value="timeVal" format="yyyy/MM/dd"
          type="datetimerange" placement="bottom-end" placeholder="请选择时间" class="input-width" :options="options">
        </DatePicker>
      </FormItem>
      <FormItem label="用户名称：">
        <Input placeholder="请输入用户名称" v-model="formValidate.nickname" class="input-add" @on-search="userSearchs"></Input>
        <Button type="primary" @click="userSearchs()">查询</Button>
      </FormItem>
    </Form>
    <Table :loading="loading2" highlight-row no-userFrom-text="暂无数据" no-filtered-userFrom-text="暂无筛选结果" ref="selection"
      :columns="columns4" :data="tableList2" height="500">
    ...
    <div class="acea-row row-right page">
      <Page :current="formValidate.page" :total="total2" show-elevator show-total @on-change="pageChange2"
...
export default {
  ...
  mounted() {
    if (this.is_channel !== null) {
      this.formValidate.is_channel = this.is_channel;
    }
    this.getListService();
  },
  methods: {
