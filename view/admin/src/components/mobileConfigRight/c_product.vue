  ...
  <div class="c_product" v-if="configData">
          ...
          <div class="content">
            <div class="con-item" v-for="(list, key) in item.chiild" :key="key">
              ...
              <div style="width: 100%" @click="getLink(index, key, item)">
                <Input
                  :icon="list.title == '链接' ? 'ios-link' : 'ios-arrow-forward'"
                  :show-word-limit="!key ? true : false"
            ...
            <div class="con-item" v-if="item.link">
              ...
              <Select
                v-model="item.link.activeVal"
                style=""
                @on-change="sliderChange(index)"
              >
                <Option
    ...
    <div v-if="configData.list">
      <div
        class="add-btn"
        @click="addHotTxt"
        v-if="configData.list.length < configData.max"
      >
  ...
  methods: {
    ...
    addHotTxt() {
      if (this.configData.list.length == 0) {
        ...
        let obj = JSON.parse(
          JSON.stringify(this.configData.list[0])
        );
        obj.chiild[0].val = "";
        obj.chiild[1].val = "";
