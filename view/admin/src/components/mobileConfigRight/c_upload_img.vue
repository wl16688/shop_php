<template>
  <div
    class="upload_img"
    :class="configData.type == 'code' ? 'on' : ''"
    v-if="configData"
  >
    <div class="header" v-show="configData.header">{{ configData.header }}</div>
    <div class="title">{{ configData.title }}</div>
    <div class="list">
      <div class="item">
        <div class="name">
          {{ configData.type == "code" ? configData.name : "图片" }}
        </div>
        <div class="picTxt">
      ...
      <div v-if="configData.type != 'code'" class="item">
        <div class="name">链接</div>
        <Input
          v-model="configData.link"
          icon="ios-link"
          @on-click="getLink()"
          search
          placeholder="输入链接"
